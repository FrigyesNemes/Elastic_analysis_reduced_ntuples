...
int draw(int run, string histoname1)
{
  ...
  TFile *myfile2 = TFile::Open((directory + "output_LBRT_run_324536.0_re_reco_Totem1.root").c_str()) ;
  ...
  string histoname2 = histoname1 ;

  TH2F *temp1 = (TH2F *)myfile->Get(histoname1.c_str()) ;
  TH2F *temp2 = (TH2F *)myfile2->Get(histoname2.c_str()) ;
  ...
  temp1->SetTitle(("compare run=" + ss.str() + " and run_{ref}=324536 using histogram: " + histoname1 ).c_str()) ;
